...
def click_all_voice_buttons(driver):
    print("🔍 Clicking 'Play voice' buttons...")
    clicked = 0
    seen = set()
    ...
    for _ in range(10):
        ...
        for btn in buttons:
            try:
                identifier = str(btn.location) + str(btn.size)
    ...
    print(f"Finished clicking {clicked} buttons.\n")
...
def main():
    ...
    try:
        ...
        click_all_voice_buttons(driver)

        # Download captured URLs
        urls = driver.execute_script("return window.capturedAudioUrls || []")
        print(f"\nFound {len(urls)} captured MP3 URLs in memory.")
